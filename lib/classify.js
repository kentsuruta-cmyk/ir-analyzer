...
const RE_KABUNUSHI = /(株主通信|業績のご報告|事業報告書|アニュアルレポート|統合報告書|招集ご?通知)/;
// 「修正」は含めない。決算短信は本文に「業績予想の修正」と書くのが普通で、
// 修正まで拾うと本文判定で全部の短信が「訂正決算短信」になって捨てられていた。
const RE_CORRECTION = /訂正/;
...
// 和暦（平成25年3月期・令和元年度 など）を西暦に直す。古いIRサイトは和暦表記が普通で、
// 読めないと決算期が「不明」になり、古い資料の足切りも標準セットの新旧判定も効かない
// （平成29年の短信が「最新の決算短信」として標準セットに選ばれていた）。
const ERA_BASE = { 明治: 1867, 大正: 1911, 昭和: 1925, 平成: 1988, 令和: 2018 };
export function warekiToSeireki(text) {
  return (text || "").replace(/(明治|大正|昭和|平成|令和)\s*(元|\d{1,2})\s*年/g, (_, era, n) =>
...
export function classifyFromLabel(label) {
  ...
  const text = warekiToSeireki(toHalfWidth(rawText));
