...
const MetricsSchema = z.object({
  ...
  periods: z.array(
    z.object({
      ...
      // 前年同期どうしを突き合わせるための分解。表記の揺れをここで吸収する。
      fiscal_year: z.string(),       // "2027年3月期"（決算期。年度表記でも必ず「〇〇年〇月期」に直す）
      quarter_label: z.enum(["1Q", "2Q", "3Q", "4Q", "通期"]),
      is_cumulative: z.boolean(),    // 累計なら true、単独四半期なら false（通期は true）
    })
  ),
  rows: z.array(
    z.object({
...
6-1. fiscal_year は必ず「2027年3月期」の形に正規化する（「2026年度」と書かれていても決算月を判断して直す）。
   quarter_label は 1Q/2Q/3Q/4Q/通期 のいずれか。「第2四半期累計」なら 2Q かつ is_cumulative=true、
   通期は quarter_label="通期" かつ is_cumulative=true にする。
   これは前年同期どうしを突き合わせるために使うので、同じ四半期には必ず同じ値を入れること。
6-2. 決算短信の1ページ目には「前年同期」の実績が並記されている。**その前年同期の数値も必ず
   独立した period として作ること**（例: 2027年3月期1Qの短信に載っている2026年3月期1Qの実績）。
   これが無いと前年同期比を並べられない。
...
決算短信に並記されている「前年同期」の実績も、独立した期として必ず取り出してください（前年同期比を並べるために使います）。
