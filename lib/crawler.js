...
async function collectPdfLinksFromDom(page, pageUrl, foundMap) {
  ...
  for (const a of anchors) {
    ...
    // リンク自身の文字がいちばん具体的なので、それが使えるならそれを使う。
    // 使えないとき（「PDF」「詳細」など）だけ、セクション名や表の文脈で補う。
    //
    // 以前は「セクション名＋表の文脈＋リンク文字」を常に連結し、同じURLに複数の
    // 候補が出たら“長いほう”を採用していた。アコーディオンを開くたびに収集し直す
    // サイト（じげんのIRニュースは19回クリックする）では、隣の資料の文言まで
    // 繋がったラベルがどんどん育ち、48件すべてが同じ種別に化けていた。
    const own = (a.text || "").trim();
    const composed = [a.section, a.context, a.text || a.title].filter(Boolean).join(" ").trim();
    const label = ((own.length >= 6 ? own : composed) || fileNameOf(absolute)).slice(0, 120);
    const group = a.section || dirOf(absolute);

    const existing = foundMap.get(absolute);
    // 同じURLに複数の候補が出たら、リンク自身の文字があるものを優先し、
    // どちらもある/ないなら短いほうを採る（長いほど別資料の文言が混ざっている）。
    const better = () => {
      if (!existing) return true;
      const hadOwn = existing.fromOwnText;
      const hasOwn = own.length >= 6;
      if (hasOwn !== hadOwn) return hasOwn;
      return label.length < existing.label.length;
    };
    if (better()) {
      foundMap.set(absolute, { label, group, fromOwnText: own.length >= 6 });
    }
  }
